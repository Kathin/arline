...
gulp.task('css:loader', ['css'], () => {
  return gulp.src('./dev/css/blocks/page-loader.css')
    .pipe($.plumber())
    .pipe($.cssnano())
...
gulp.task('img', () => {
  return gulp.src('./dev/img/**/*.+(jpg|png|svg)')
    .pipe($.plumber())
...
gulp.task('html', () => {
  walk(path.join(__dirname, 'dev/stylus'), (file) => {
